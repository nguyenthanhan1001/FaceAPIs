...
def _process_image(directory, name):
    ...
    for obj in root.findall('object'):
        ...
        #     rxmin = random.random()
        #     rymin = random.random();
        #     rxmax = rxmin + (1.0-rxmin)*random.random()
        #     rymax = rymin + (1.0-rymin)*random.random()
            
        #     bboxes.append(rymin, rxmin, rymax, rxmax)
        # else:
        a, b, c, d = float(bbox.find('ymin').text) / shape[0],float(bbox.find('xmin').text) / shape[1], \
                    float(bbox.find('ymax').text) / shape[0], float(bbox.find('xmax').text) / shape[1]
                    
        if np.max([a, b, c, d]) <= 1 and np.min([a, b, c, d]) >= 0:
            label = obj.find('name').text
            id = int(VOC_LABELS[label][0])
            labels.append(id)
...
def run(dataset_dir, output_dir, name='voc_train', shuffling=False):
    ...
    print '\nFinished converting the facescrub dataset in {0} s'.format(time.time()-xtime) # 26.6s
